...
- Each Perl package/module must reside in its own `.pm` file
- Scripts (containing `main` or executable logic) reside in `.pl` files
- Do not define multiple packages in a single file

## Indentation and Braces

- Whitesmiths indentation style with hard tabs (never spaces)
- Hard tabs are 8 spaces wide
- `else` on its own line
- Named subs
  - Opening brace `{` on line after `sub`, not indented
  - Closing brace `}` not indented
  - Code inside starts at first character with no indentation
- Anonymous subs
  - Simple one-liners: `my $sub = sub { my ($x) = @_ ; return $x * 2 ; } ;`
  - Complex multi-line
    - Opening `{` and closing `} ;` aligned under or past `sub` keyword using hard tabs
    - Code inside indented relative to the opening brace
- Multi-line return
  - `return` alone
  - Opening `(` on next line indented
  - Values indented
  - Closing `)` indented with space before `;`
...
- Multi-line calls
  ...
  - Indentation uses hard tabs to align with the start of the function name
  - Ensure at least one level of indentation (1 tab)
  - One argument per line with trailing comma
  - Closing `)` indented to align with the opening `(`
...
- Prefer separate statements over chaining if possible
...
- Indent new lines with hard tabs only
- Align characters on the same line using spaces only

## Multi-line Construct Alignment Principle

- Indent argument lists under the parent construct
- Indent blocks under the parent construct
- Indent method chains under the parent construct
- Indent regex patterns under the parent construct
- Indent all multi-line constructs under the parent construct
- Create visual hierarchy showing ownership/belonging
...
- Don't wrap lines
- Be concise
- List entries start with a lower case
- List entries don't have a terminating dot
```
